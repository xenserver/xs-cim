...
# This script is designed to use the BITS interface with a mechanism that uses a file copy process.  The output being a true VHD.
# The desired outcome of this script is that a VM is export with a single VHD (regardless of snapshots)

###  The current implemenation does not support this   ###
...
$loopCount = 1	# The number of time to loop through the test for repeated stress.
...
"@
	...
	#		$destination = "Q:\" + $element.Xen_DiskSettingData.AddressOnParent + "." + $element.Xen_DiskSettingData.HostExtentName + ".vhd"
			...
			$source = $transferVm.Xen_ConnectToDiskImageJob.TargetURI + ".vhd"
			...
			{
				"Transferred" {
					$bitsTime = $transferJob.TransferCompletionTime - $transferJob.CreationTime
...
"@
	...
	#	$source =  "Q:\" + $element.Name
		...
		$transferJob = Start-BitsTransfer -Source $source -destination $destination -Asynchronous -DisplayName XenVdiTransfer -TransferType Upload

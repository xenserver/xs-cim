...
# Currently this requires creating a new vm from a template such as the 
# Demo Linux VM that is included withteh Linux Supplimental Pack.
# If using the TransferVM Temaplate be sure to 'clear' the other-config parameters using 'xe'
...
# Create new virtual machines from the source until we reach the total count passed.	
	...
	while ($a -lt $vmCount) {
		$vmName = $source + $a
		$captureTheOutput = CopyVm $vmName $refVmInstanceId $xenSrInstanceId
		$a++;$a
		}

...
IF (%2)==() GOTO ENUM_NO_FILTER
 call winrm enum http://schemas.citrix.com/wbem/wscim/1/cim-schema/2/%1 -r:%server% -encoding:utf-8 -a:basic -u:%user% -p:%pass% -filter:%filter% 
 GOTO ENUM_END
:ENUM_NO_FILTER
 call winrm enum http://schemas.citrix.com/wbem/wscim/1/cim-schema/2/%1 -r:%server% -encoding:utf-8 -a:basic -u:%user% -p:%pass%
:ENUM_END

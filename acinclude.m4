dnl
dnl $Id: acinclude.m4,v 1.1.1.1 2005/08/30 23:08:51 bestor Exp $
dnl
 dnl 
...
AC_DEFUN([_CHECK_CMPI],
	...
	[
		...
		CMNewString(&broker, "TEST", &status);
	],
	[
		have_CMPI=yes
	...
	if test "$have_CMPI" == "yes"; then
	  ...
	  _DIRS_="/usr/include/cmpi \
		  ...
		  /usr/include/Pegasus/Provider/CMPI \
                  /usr/src/sblim-sfcb-1.3.0 \
		  /usr/include/openwbem \
                  /usr/local/include/openwbem \
...
AC_DEFUN([CHECK_TESTSUITE],
	...
	_DIRS="$datadir/sblim-testsuite $TESTSUITEDIR"
	save_exec_prefix=${exec_prefix}
	save_prefix=${prefix}
	if test xNONE == x${prefix}; then
		prefix=/usr/local
